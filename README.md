...
| 播放 | 底部播放条（进度拖动、音量、上下首、列表/单曲/不循环、随机）、音质切换、**手动切源**、断点续传的 Range 代理；**下次打开自动回到上次的队列与进度**（停在那首歌的暂停状态，点一下续播） |
| 多设备 | 同账号设备互相可见（一条 SSE 长连接）；远程控制进度 / 音量、把队列连进度搬到另一台设备；**双向跟随**——任意一台可镜像另一台同步播放（时钟校正 + 速率微调，实际稳在 ±100~300ms）；设备状态只在内存里，不落库 |
